...
I recently submitted my project and was asked to resubmit to address the fact that I had some instances where one car had two overlaying bounding boxes. I've found and corrected an error in my pipeline in the way the color histogram bins are generated, removed a 48x48 sliding window, and increased sliding window overlap. These changes noticeably improved classification and detection accuracy. I've also implemented a four frame filter to smooth the detection boxes and help reduce false positives. Unfortunately, it is taking a long time to generate the project video and I'm concerned that I'll miss the end of term deadline. I'm submitting a small portion of the video with the new pipeline in this [link](./output_images/project_video_labeled_short.mp4), along with the full video from my original pipeline prior to resubmittal in this [link](./output_images/project_video_labeled.mp4). The submitted small portion demonstrates a part of the video where my previous pipeline either could not identify the white car or identified it with multiple boxes. I'll continue to process the full video in case this is not acceptable. The video pipeline function, "process_image", can be found in (cell 13)
...
The first step is to create a heatmap from the boxes identified in the sliding windows search. The heatmap is created by adding (1) to the location of each pixel for each box that overlaps with it. I also combine the current heatmap with heatmaps from the previous four images in order to help reject false positives and smooth out the bounding box.

Next, I apply a threshold to the heatmap to reduce false positives. Any pixel in the heatmap which has a value below the threshold will be set to zero. I found a threshold of (6) to work well for my classifier.
